...
function injectScript(content) {
  try {
    ...
    console.error('Larecoin: Provider injection failed.', error);
  }
}
...
async function setupStreams() {
  ...
  pump(pageMux, pageStream, pageMux, (err) =>
    logStreamDisconnectWarning('Larecoin Inpage Multiplex', err),
  );
  pump(extensionMux, extensionStream, extensionMux, (err) => {
    logStreamDisconnectWarning('Larecoin Background Multiplex', err);
  ...
  pump(legacyPageMux, legacyPageStream, legacyPageMux, (err) =>
    logStreamDisconnectWarning('Larecoin Legacy Inpage Multiplex', err),
  );
  pump(
    legacyExtensionMux,
    extensionStream,
    ...
    legacyExtensionMux,
    (err) => {
      logStreamDisconnectWarning('Larecoin Background Legacy Multiplex', err);
      notifyInpageOfStreamFailure();
    },
  );
...
function forwardTrafficBetweenMuxes(channelName, muxA, muxB) {
  ...
  pump(channelA, channelB, channelA, (error) =>
    console.debug(
      `Larecoin: Muxed traffic for channel "${channelName}" failed.`,
      error,
    ),
  );
}
...
function forwardNamedTrafficBetweenMuxes(
  ...
  pump(channelA, channelB, channelA, (error) =>
    console.debug(
      `Larecoin: Muxed traffic between channels "${channelAName}" and "${channelBName}" failed.`,
      error,
    ),
  );
}
...
function logStreamDisconnectWarning(remoteLabel, error) {
  console.debug(
    `Larecoin: Content script lost connection to "${remoteLabel}".`,
    error,
  );
}
...
function redirectToPhishingWarning() {
  console.debug('Larecoin: Routing to Phishing Warning component.');
