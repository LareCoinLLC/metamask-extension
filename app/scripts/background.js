 ...
 * @property {string} provider.type - An identifier for the type of network selected, allows Larecoin to use custom provider strategies for known networks.
 ...
 * @property {MetaMaskState} data - The data emitted from Larecoin controller, or used to initialize it.
 * @property {Number} version - The latest migration version that has been run.
 */

/**
 * Initializes the Larecoin controller, and sets up all platform configuration.
...
async function initialize() {
  ...
  log.debug('Larecoin initialization complete.');
}
 ...
 * @returns {Promise<MetaMaskState>} Last data emitted from previous instance of Larecoin.
 */
async function loadStateFromPersistence() {
  ...
  if (versionedData && !versionedData.data) {
    ...
    sentry.captureMessage('Larecoin - Empty vault found - unable to recover');
  }
  ...
  if (!versionedData) {
    throw new Error('Larecoin - migrator returned undefined');
  }
  ...
  if (localStore.isSupported) {
    ...
    setTimeout(() => {
      throw new Error('Larecoin - Localstore not supported');
    });
  }
...
/**
 * Initializes the Larecoin Controller with any initial state and default language.
...
function setupController(initState, initLangCode) {
  ...
  pump(
    ...
    (error) => {
      log.error('Larecoin - Persistence pipeline failed', error);
    },
  );
  ...
  async function persistData(state) {
    if (!state) {
      throw new Error('Larecoin - updated state is missing');
    }
    if (!state.data) {
      throw new Error('Larecoin - updated state does not have data');
    }
    if (localStore.isSupported) {
      try {
  ...
  /**
   * Connects a Port to the Larecoin controller via a multiplexed duplex stream.
   * This method identifies trusted (Larecoin) interfaces, and connects them differently from untrusted (web pages).
...
// On first install, open a new tab with Larecoin
