...
export default class EncryptionPublicKeyManager extends EventEmitter {
  ...
  addUnapprovedMessageAsync(address, req) {
    return new Promise((resolve, reject) => {
      if (!address) {
        reject(new Error('Larecoin Message: address field is required.'));
        return;
      }
          ...
          case 'rejected':
            reject(
              ethErrors.provider.userRejectedRequest(
                'Larecoin EncryptionPublicKey: User denied message EncryptionPublicKey.',
              ),
            );
            return;
          default:
            reject(
              new Error(
                `Larecoin EncryptionPublicKey: Unknown problem: ${JSON.stringify(
                  address,
                )}`,
              ),
            );
   ...
   *
   * @param {Object} msgParams The msgParams to be used when eth_getEncryptionPublicKey is called, plus data added by Larecoin.
