...
function detectAndRunEntryTask() {
  ...
  if (!taskName) {
    throw new Error(`Larecoin build: No task name specified`);
  }
...
async function runTask(taskName, { skipStats } = {}) {
  if (!(taskName in tasks)) {
    throw new Error(`Larecoin build: Unrecognized task name "${taskName}"`);
  }
  if (!skipStats) {
  ...
  try {
    ...
    console.error(
      `Larecoin build: Encountered an error while running task "${taskName}".`,
    );
    console.error(err);
    process.exit(1);
  }
...
function createTask(taskName, taskFn) {
  if (taskName in tasks) {
    throw new Error(
      `Larecoin build: task "${taskName}" already exists. Refusing to redefine`,
    );
  }
...
function runInChildProcess(task) {
  ...
  if (!taskName) {
    throw new Error(
      `Larecoin build: runInChildProcess unable to identify task name`,
    );
  }
    ...
    await new Promise((resolve, reject) => {
      childProcess.once('exit', (errCode) => {
        if (errCode !== 0) {
          reject(
            new Error(
              `Larecoin build: runInChildProcess for task "${taskName}" encountered an error ${errCode}`,
            ),
          );
          return;
        }
